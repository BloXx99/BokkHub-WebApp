...
if(isset($_SESSION['search_author']))
{
    ...
    $sql = "SELECT * FROM authors WHERE name_surname LIKE '%{$input}%';";
    $how_many = "SELECT COUNT(*) as entries FROM authors WHERE name_surname LIKE '%{$input}%';";
              ...
              <?php
                ...
                if($result){
                  while($row = $result->fetch_assoc()){
                    ...
                    echo '<tr>
                    <th scope = "row">'.$id.'</th>
                    <td><a href ="../display_author.php?author_id='.$id.'">'.$name.'</a></td>
                    <td>'.$born.'</td>
                    <td>'.$nationality.'</td>
                    </tr>';
